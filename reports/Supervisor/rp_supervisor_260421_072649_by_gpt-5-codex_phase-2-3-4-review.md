...
- backend analyze job status still includes `cloning` at [`gitnexus/src/server/analyze-job.ts:24`](F:\GitNexus-main\gitnexus\src\server\analyze-job.ts:24)
- `/api/analyze` still marks every accepted local-path job as `cloning` before analysis starts at [`gitnexus/src/server/api.ts:1155`](F:\GitNexus-main\gitnexus\src\server\api.ts:1155)
- the active web client status type still includes `cloning` at [`gitnexus-web/src/services/backend-client.ts:61`](F:\GitNexus-main\gitnexus-web\src\services\backend-client.ts:61)
- the active progress UI still renders `cloning` as `Cloning repository` and `pulling` as `Pulling latest` at [`gitnexus-web/src/components/AnalyzeProgress.tsx:10`](F:\GitNexus-main\gitnexus-web\src\components\AnalyzeProgress.tsx:10)
...
- `startAnalyze()` still accepts `{ url?: string; path?: string; ... }` at [`gitnexus-web/src/services/backend-client.ts:664`](F:\GitNexus-main\gitnexus-web\src\services\backend-client.ts:664)
- `JobStatus` still exposes `repoUrl?: string` at [`gitnexus-web/src/services/backend-client.ts:59`](F:\GitNexus-main\gitnexus-web\src\services\backend-client.ts:59)
...
- `RepoAnalyzer` submits `{ path: ... }` at [`gitnexus-web/test/unit/RepoAnalyzer.local-only.test.tsx:36`](F:\GitNexus-main\gitnexus-web\test\unit\RepoAnalyzer.local-only.test.tsx:36)

But the backend-side Phase 3 test coverage is still too narrow:

- `gitnexus/test/unit/analyze-api.test.ts` only tests `resolveAnalyzeRepoPath()` helper behavior at [`gitnexus/test/unit/analyze-api.test.ts:1`](F:\GitNexus-main\gitnexus\test\unit\analyze-api.test.ts:1)
...
- app imports `useAppState.local-runtime` and `SettingsPanel.local-runtime` directly at [`gitnexus-web/src/App.tsx:1`](F:\GitNexus-main\gitnexus-web\src\App.tsx:1)
...
- CLI help describes local runtime / MCP surfaces at [`gitnexus/src/cli/index.ts:15`](F:\GitNexus-main\gitnexus\src\cli\index.ts:15)
...
- onboarding now points to `gitnexus serve` without `gitnexus@latest` fallback, covered at [`gitnexus-web/test/unit/OnboardingGuide.local-only.test.tsx:5`](F:\GitNexus-main\gitnexus-web\test\unit\OnboardingGuide.local-only.test.tsx:5)
- loopback-only backend URL enforcement is covered in `useBackend.local-only.test.tsx` and `server-connection.test.ts`
- loopback-only CORS policy is covered in `gitnexus/test/unit/cors.test.ts`

## Validation run

- `cd gitnexus && npx tsc --noEmit` — pass
- `cd gitnexus && npx vitest run test/unit/analyze-api.test.ts test/unit/analyze-job.test.ts test/unit/mcp-runtime-alignment.test.ts test/unit/tool-runtime-alignment.test.ts test/unit/serve-command.test.ts test/unit/wiki-gated.test.ts test/unit/setup.test.ts test/unit/setup-codex.test.ts test/unit/cors.test.ts` — pass (`43/43`)
- `cd gitnexus-web && npx tsc -b --noEmit` — pass
- `cd gitnexus-web && npm test` — pass (`247/247`)
