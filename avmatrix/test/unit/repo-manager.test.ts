...
describe('getGlobalDir', () => {
  it('uses ~/.avmatrix by default', async () => {
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('avmatrix-config-test-');
...
describe('registerRepo name override + collision guard (#829)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('avmatrix-registry-home-');
    tmpRepoA = await createTempDir('avmatrix-repo-a-');
    tmpRepoB = await createTempDir('avmatrix-repo-b-');
  ...
  it('basename collisions without an explicit --name still register silently (backward-compat)', async () => {
    ...
    const parentA = await createTempDir('avmatrix-collide-parent-a-');
    const parentB = await createTempDir('avmatrix-collide-parent-b-');
...
describe('getInferredRepoName + registerRepo (#979 — git remote inference)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('avmatrix-registry-home-979-');
    savedAvmatrixHome = process.env.AVMATRIX_HOME;
    process.env.AVMATRIX_HOME = tmpHome.dbPath;
  });
  ...
  it('getInferredRepoName returns null when there is no .git directory', async () => {
    const tmp = await createTempDir('avmatrix-no-git-');
    try {
  ...
  it('getInferredRepoName returns null when origin is unset', async () => {
    const tmp = await createTempDir('avmatrix-no-origin-');
    try {
  ...
  it('getInferredRepoName returns the remote repo name when origin is set', async () => {
    const tmp = await createTempDir('avmatrix-with-origin-');
    try {
  ...
  it('registerRepo derives name from git remote when basename is generic (Gas-Town repro)', async () => {
    ...
    const root = await createTempDir('avmatrix-gastown-');
    try {
  ...
  it('two analyze calls of differently-remoted "rig" leaves no longer collide', async () => {
    ...
    const root = await createTempDir('avmatrix-gastown-2-');
    try {
  ...
  it('explicit --name still wins over remote inference', async () => {
    const tmp = await createTempDir('avmatrix-name-wins-');
    try {
  ...
  it('preserved alias still wins over remote inference on re-analyze', async () => {
    const tmp = await createTempDir('avmatrix-preserve-alias-');
    try {
      initGitRepo(tmp.dbPath, 'https://github.com/owner/from-remote.git');
  ...
  it('falls back to basename when no .git / no remote is available', async () => {
    const tmp = await createTempDir('avmatrix-fallback-basename-');
    try {
      const name = await registerRepo(tmp.dbPath, meta);
