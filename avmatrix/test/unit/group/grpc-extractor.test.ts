...
describe('GrpcExtractor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'avmatrix-grpc-'));
  ...
  const makeRepo = (repoPath: string): RepoHandle => ({
    ...
    path: 'test/app',
    repoPath,
    storagePath: path.join(repoPath, '.avmatrix'),
  });
...
describe('GrpcExtractor.extract ambiguous proto resolution', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'avmatrix-grpc-ambig-'));
    extractor = new GrpcExtractor();
  });
  afterEach(async () => {
    await fsp.rm(tmpDir, { recursive: true, force: true });
