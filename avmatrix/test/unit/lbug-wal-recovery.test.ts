...
describe('lbug WAL recovery', () => {
  ...
  it('removes .wal and .lock sidecars then retries when schema creation hits WAL corruption', async () => {
    ...
    await expect(adapter.initLbug('repos/demo/.avmatrix/lbug')).resolves.toBeDefined();

    expect(databaseCtorMock).toHaveBeenCalledTimes(2);
    expect(connectionCtorMock).toHaveBeenCalledTimes(2);
    expect(unlinkMock).toHaveBeenCalledWith('repos/demo/.avmatrix/lbug.wal');
    expect(unlinkMock).toHaveBeenCalledWith('repos/demo/.avmatrix/lbug.lock');
  });
  ...
  it('retries initialization when VECTOR extension load hits WAL corruption', async () => {
    ...
    await expect(adapter.initLbug('repos/vector/.avmatrix/lbug')).resolves.toBeDefined();

    expect(databaseCtorMock).toHaveBeenCalledTimes(2);
    expect(unlinkMock).toHaveBeenCalledWith('repos/vector/.avmatrix/lbug.wal');
    expect(unlinkMock).toHaveBeenCalledWith('repos/vector/.avmatrix/lbug.lock');
  });
});
