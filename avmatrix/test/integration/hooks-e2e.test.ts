...
describe.each(HOOKS)('hooks e2e ($name)', ({ name: _name, path: hookPath }) => {
  ...
  describe('PreToolUse — silent without avmatrix CLI', () => {
    // PreToolUse tries to spawn `avmatrix augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `avmatrix augment` hangs and the 10s timeout kills the process.
