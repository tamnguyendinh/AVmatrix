...
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), AVMATRIX_DIR);
};
...
export const getGlobalDir = (): string => {
  return process.env[AVMATRIX_HOME_ENV] || path.join(os.homedir(), AVMATRIX_DIR);
};
...
export interface CLIConfig {
  ...
  cursorModel?: string;
  /** Optional cap for materialized execution flows during analyze. */
  maxProcesses?: number;
