...
function printPerformanceSummary(performance: AnalyzePerformanceReport): void {
  ...
  if (resolutionTimings) {
    ...
    console.log(
      `  resolution detail: total ${phaseTotalMs.toFixed(1)}ms | references ${(resolutionTimings.referenceResolveMs ?? 0).toFixed(1)}ms | init ${(resolutionTimings.readonlyIndexInitMs ?? 0).toFixed(1)}ms | worker ${(resolutionTimings.referenceWorkerResolveMs ?? 0).toFixed(1)}ms | merge ${(resolutionTimings.referenceMergeMs ?? 0).toFixed(1)}ms | index ${(resolutionTimings.referenceIndexBuildMs ?? 0).toFixed(1)}ms | emit ${(resolutionTimings.graphEmitMs ?? 0).toFixed(1)}ms | sites ${resolutionCounters?.scopeResolutionReferenceSites ?? 0} | chunks ${resolutionCounters?.scopeResolutionChunks ?? 0} | workers ${resolutionCounters?.scopeResolutionUsedWorkers ? (resolutionCounters.scopeResolutionWorkerCount ?? 0) : 0} | indexBytes ${formatBytes(resolutionCounters?.scopeResolutionReadonlyIndexBytes ?? 0)} | indexScopes ${resolutionCounters?.scopeResolutionReferenceIndexSourceScopes ?? 0} | indexDefs ${resolutionCounters?.scopeResolutionReferenceIndexTargetDefs ?? 0} | resolved ${resolutionCounters?.scopeResolutionResolvedReferences ?? 0} | unresolved ${resolutionCounters?.scopeResolutionUnresolvedReferences ?? 0} | emitted ${resolutionCounters?.scopeResolutionEdgesEmitted ?? 0} | finalizedImports ${resolutionCounters?.scopeResolutionFinalizedImportsEmitted ?? 0} | finalizedImportUses ${resolutionCounters?.scopeResolutionFinalizedImportUsesEmitted ?? 0} | skippedDuplicate ${resolutionCounters?.scopeResolutionDuplicateEdgesSkipped ?? 0} | skippedImportDuplicate ${resolutionCounters?.scopeResolutionDuplicateImportsSkipped ?? 0} | skippedImportUseDuplicate ${resolutionCounters?.scopeResolutionDuplicateImportUsesSkipped ?? 0}`,
    );
  }
}
