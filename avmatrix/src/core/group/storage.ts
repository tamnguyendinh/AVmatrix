...
export function getDefaultAVmatrixDir(): string {
  return getGlobalDir();
}

export function getGroupsBaseDir(avmatrixDir?: string): string {
  return path.join(avmatrixDir || getDefaultAVmatrixDir(), 'groups');
}
...
export function getGroupDir(avmatrixDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(avmatrixDir, 'groups', groupName);
}
...
export async function listGroups(avmatrixDir?: string): Promise<string[]> {
  const groupsDir = getGroupsBaseDir(avmatrixDir);
  try {
...
export async function createGroupDir(
  ...
  const groupDir = getGroupDir(avmatrixDir, groupName);
