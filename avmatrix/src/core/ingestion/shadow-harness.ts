 ...
 *     `avmatrix/shadow-parity-dashboard/` can render it offline.
 *
 * ## What it does NOT do
 *
 ...
 * ## Activation
 *
 * `AVMATRIX_SHADOW_MODE=1` (or `'true'`, `'yes'`, case-insensitive,
...
export interface ShadowHarness {
  /** `true` iff `AVMATRIX_SHADOW_MODE` is truthy. When `false`, `record()` is a no-op. */
...
/**
 * Construct a harness. Reads `AVMATRIX_SHADOW_MODE` at construction time
...
export function createShadowHarness(): ShadowHarness {
  const enabled = parseShadowModeEnv(process.env['AVMATRIX_SHADOW_MODE']);
...
/**
 * Env-var parser for `AVMATRIX_SHADOW_MODE`. Accepts the same truthy
