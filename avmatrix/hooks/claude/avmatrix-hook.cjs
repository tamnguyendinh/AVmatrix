...
/**
 * Resolve the avmatrix CLI path.
 * 1. Relative path (works when script is inside npm package)
 * 2. require.resolve (works when avmatrix is globally installed)
...
function resolveCliPath() {
  ...
  if (!fs.existsSync(cliPath)) {
    try {
      cliPath = require.resolve('avmatrix/dist/cli/index.js');
    } catch {
      cliPath = '';
    }
  }
 ...
 */
function runAVmatrixCli(cliPath, args, cwd, timeout) {
...
function handlePreToolUse(input) {
  ...
  let result = '';
  try {
    const child = runAVmatrixCli(cliPath, ['augment', '--', pattern], cwd, 7000);
