 ...
 *   tree-sitter-proto is vendored under avmatrix/vendor/tree-sitter-proto/
 ...
 *   avmatrix's own optionalDependencies, and moved native compilation here.
 *
 * What this does:
 ...
 *   warn and exit 0 so avmatrix install still succeeds.
 */
const fs = require('fs');
...
try {
  ...
  console.warn(
    '[tree-sitter-proto] Proto (.proto) parsing will be unavailable. Non-proto avmatrix functionality is unaffected.',
  );
  // Exit 0: optionalDependency failures must not fail the avmatrix install.
  process.exit(0);
}
