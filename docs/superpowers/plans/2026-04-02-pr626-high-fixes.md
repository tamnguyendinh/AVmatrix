...
**Paths:** All file paths are relative to the monorepo root (`AVmatrix/`). Git commands run from the root. The `avmatrix/` prefix is a package subdirectory, not a separate repo.
...
**Files:**
- Modify: `avmatrix/src/core/group/storage.ts:17-19` (getGroupDir) and `:63-68` (createGroupDir)
- Test: `avmatrix/test/unit/group/storage.test.ts`

- [ ] **Step 1: Write failing tests for validateGroupName**

In `avmatrix/test/unit/group/storage.test.ts`, add `createGroupDir` and `validateGroupName` to the existing import from `'../../../src/core/group/storage.js'` (line 6-11). Then add these describe blocks at the end of the outer `describe('Group storage', ...)`:
...
Run: `cd avmatrix && npx vitest run test/unit/group/storage.test.ts`
...
In `avmatrix/src/core/group/storage.ts`, add the validation function before `getGroupDir` and call it:
...
```bash
cd avmatrix && git add src/core/group/storage.ts test/unit/group/storage.test.ts
...
**Files:**
- Modify: `avmatrix/src/core/group/service-boundary-detector.ts:24-51` (add constant), `:78` (walkForBoundaries), `:130` (hasSourceFilesInSubdirs)
- Test: `avmatrix/test/unit/group/service-boundary-detector.test.ts`

- [ ] **Step 1: Write failing tests for excluded directories**

Add this describe block inside the existing `detectServiceBoundaries` describe in `avmatrix/test/unit/group/service-boundary-detector.test.ts`:
...
Run: `cd avmatrix && npx vitest run test/unit/group/service-boundary-detector.test.ts`
...
In `avmatrix/src/core/group/service-boundary-detector.ts`:
...
```bash
cd avmatrix && git add src/core/group/service-boundary-detector.ts test/unit/group/service-boundary-detector.test.ts
...
**Files:**
- Modify: `avmatrix/src/cli/group.ts:160` (remove import), `:187-189` (remove finally block body)
- Test: `avmatrix/test/unit/group/sync.test.ts` (add pool cleanup test)
- Test: `avmatrix/test/integration/group/group-cli.test.ts` (verify no blanket close in source)

- [ ] **Step 1: Write unit tests for per-id pool cleanup in sync.ts**

Add to `avmatrix/test/unit/group/sync.test.ts`, inside the existing `describe('syncGroup', ...)`:
...
Run: `cd avmatrix && npx vitest run test/unit/group/sync.test.ts`
...
Add to `avmatrix/test/integration/group/group-cli.test.ts`:
...
Run: `cd avmatrix && npx vitest run test/integration/group/group-cli.test.ts`
Expected: FAIL — `closeLbug()` (no args) exists at line 188.

- [ ] **Step 5: Remove blanket closeLbug() from cli/group.ts**

In `avmatrix/src/cli/group.ts`:
...
Run: `cd avmatrix && npx vitest run test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts`
Expected: ALL PASS

- [ ] **Step 7: Commit**

```bash
cd avmatrix && git add src/cli/group.ts test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts
...
**Files:**
- Modify: `avmatrix/src/core/group/extractors/grpc-extractor.ts:101-130` (parseProtoFile)
- Test: `avmatrix/test/unit/group/grpc-extractor.test.ts`

- [ ] **Step 1: Write failing tests for nested braces in proto services**

Add this describe block inside the existing `proto file parsing` describe in `avmatrix/test/unit/group/grpc-extractor.test.ts`:
...
Run: `cd avmatrix && npx vitest run test/unit/group/grpc-extractor.test.ts`
...
In `avmatrix/src/core/group/extractors/grpc-extractor.ts`, replace the `parseProtoFile` method (lines 101-130):
...
```bash
cd avmatrix && git add src/core/group/extractors/grpc-extractor.ts test/unit/group/grpc-extractor.test.ts
...
Run: `cd avmatrix && npx vitest run test/unit/group/ test/integration/group/`
Expected: ALL PASS

- [ ] **Step 2: Run full test suite to catch regressions**

Run: `cd avmatrix && npx vitest run`
Expected: ALL PASS, 0 failures

- [ ] **Step 3: Run typecheck**

Run: `cd avmatrix && npx tsc --noEmit`
Expected: No errors
...
Add to `avmatrix/test/integration/group/group-cli.test.ts` inside the existing `group CLI` describe:
...
```bash
cd avmatrix && git add test/integration/group/group-cli.test.ts
