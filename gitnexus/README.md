...
It is fixed in the current AVmatrix codebase. If you are using the published compatibility package, upgrade to the latest version:

```bash
npm install -g gitnexus@latest       # compatibility package still published as gitnexus
# then use the canonical command
avmatrix analyze
```
